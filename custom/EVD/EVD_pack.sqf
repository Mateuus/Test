...
_playersNear = {isPlayer _x} count (player nearEntities ["CAManBase", 5]); /****How many players are within 5 meters of the player?****/
_vehicleEmpty = (count (crew cursorTarget)) == 0; /****Does the vehicle the player is pointing at have anybody in it?****/
if ((_playersNear >= 2) || !(_vehicleEmpty)) exitWith { cutText ["\n\nYou cannot pack an object with players nearby.", "PLAIN DOWN"] }; /****If there is anybody within 5 meters of the player packing the vehicle OR if the vehicle the player is trying to pack has anybody in it, it will exit the script with an error message to the player telling him/her why.****/
