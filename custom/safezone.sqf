		...
		if (!canbuild) then {
			...
			if (isNil "don_firedEH_1") then {
				...
				don_firedEH_1 = player addEventHandler ["Fired",{
					cutText ["Proibido atirar com God on.","PLAIN DOWN", 2];
		...
		if (canbuild && _runOneTime) then {
			[] spawn {
				for "_x" from 1 to 30 do {
					if (_x >= 6) then {cutText [format ["%1 segundos para God off e Tiro on.", 31-_x], "PLAIN DOWN"];};
					sleep 1;
					if (!canbuild) exitWith {};
					if (_x == 30) then {
						cutText ["God desligado e tiro ligado!", "PLAIN DOWN"];
		...
		if (!canbuild) then {
			...
			if !(getPlayerUID player in _don_veh_crew) then {
				...
				if (_wait_time < _max_time) then {
					...
					cuttext [format ["Dono:%1. %2 segundos para liberar.", don_player_veh getVariable ["don_passengers","Owners not found"], round (_max_time - _wait_time)], "PLAIN DOWN"];
					cad_pvar_smessage = [format ["%1 esta tentando entrar em seu veiculo! %2 segundos para liberar!", name player, round (_max_time - _wait_time)], _don_veh_crew];
					...
					cuttext [format ["O veiculo agora esta liberado!"], "PLAIN DOWN"];
					cad_pvar_smessage = [format ["Um de seus veiculos esta liberado e %1 esta nele!", name player], _don_veh_crew];
					publicVariable "cad_pvar_smessage";
				};
			};
