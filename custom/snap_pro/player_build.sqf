...
"_nearestCity","_location","_dir","_classname","_item","_hasrequireditem","_missing","_hastoolweapon","_cancel","_reason","_started","_finished","_animState","_isMedic","_dis","_sfx","_hasbuilditem","_tmpbuilt","_onLadder","_isWater","_require","_text","_offset","_IsNearPlot","_isOk","_location1","_location2","_counter","_limit","_proceed","_num_removed","_position","_object","_canBuildOnPlot","_friendlies","_nearestPole","_ownerID","_findNearestPoles","_findNearestPole","_distance","_classnametmp","_ghost","_isPole","_needText","_lockable","_zheightchanged","_rotate","_combination_1","_combination_2","_combination_3","_combination_4","_combination","_combination_1_Display","_combinationDisplay","_zheightdirection","_abort","_isNear","_need","_needNear","_vehicle","_inVehicle","_requireplot","_objHDiff","_isLandFireDZ","_isTankTrap"];
...
if (count _nearestCity > 0) exitWith { DZE_ActionInProgress = false; systemChat ("You cannot build here!Não pode construir aqui");};
...
FastBuilding = ["76561198084611834","76561198084026095","76561198157732077"];
...
if (_hasrequireditem) then {
	...
	if !(_classnametmp in DZE_noRotate) then{
		...
		s_player_setVectorsForward = player addaction ["Empinar para Frente","custom\snap_pro\player_vectorChange.sqf","forward"];
		s_player_setVectorsBack = player addaction ["Empinar para Tras","custom\snap_pro\player_vectorChange.sqf","back"];
		s_player_setVectorsLeft = player addaction ["Empinar para esquerda","custom\snap_pro\player_vectorChange.sqf","left"];
		s_player_setVectorsRight = player addaction ["Empinar para a direita","custom\snap_pro\player_vectorChange.sqf","right"];
		s_player_setVectors1 = player addaction ["Aumentar 1 grau","custom\snap_pro\player_vectorChange.sqf","1"];
		s_player_setVectors5 = player addaction ["Aumentar 5 graus","custom\snap_pro\player_vectorChange.sqf","5"];
		s_player_setVectors45 = player addaction ["Aumentar 45 graus","custom\snap_pro\player_vectorChange.sqf","45"];
		s_player_setVectors90 = player addaction ["Aumentar 90 graus","custom\snap_pro\player_vectorChange.sqf","90"];
	};
	...
	while {_isOk} do {
		...
		if(_location1 distance _location2 > 10) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = "Voce se moveu muito de onde comecou a construir (10 metros)";
			detach _object;
			deleteVehicle _object;
			detach _objectHelper;
			deleteVehicle _objectHelper;
		...
		if(_location1 distance _objectHelperPos > 10) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = "Objeto foi colocado muito longe de onde voce comecou a construir (10 metros)";
			detach _object;
			deleteVehicle _object;
			detach _objectHelper;
			deleteVehicle _objectHelper;
		...
		if(abs(_objHDiff) > 10) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = "Nao pode mover para cima ou para baixo mais do que 10 metros";
			detach _object;
			deleteVehicle _object;
			detach _objectHelper;
			deleteVehicle _objectHelper;
	...
	if (!DZE_BuildOnRoads) then {
		if (isOnRoad _position) then { _cancel = true; _reason = "Proibido construir na rua."; };
	};

	if(!canbuild) then { _cancel = true; _reason = "Proibido construir na trade."; };
				...
				if(_lockable > 1) then {
					...
					if(_tmpbuilt isKindOf "Land_Fire_DZ") then {
						...
						if (_isPole) then {
							...
							_tmpbuilt setVariable ["plotfriends", _friends, true];
