...
if(isNil "Z_AdvancedTradingInit")then{
	...
	Z_getContainer = {
		...
		(_dialog displayCtrl 7404) ctrlSetText format["Espaco livre: 0 / 0 / 0"];
		Z_clearBuyingList;
		Z_clearLists;
		Z_SellableArray = [];
		Z_SellArray = [];
		...
		if(Z_Selling)then{
			switch (_lbIndex) do {
				case 0: { 
					["Vendendo da mochila."] call Z_filleTradeTitle;
					Z_SellingFrom = 0;
					call Z_getBackpackItems; 
				};
				case 1: { 
					["Vendendo do veiculo."] call Z_filleTradeTitle;
					Z_SellingFrom = 1;
					call Z_getVehicleItems; 
				};
				case 2: { 
					["Vendendo do inventario."] call Z_filleTradeTitle;
					Z_SellingFrom = 2;
					call Z_getGearItems; 
				};
			};
			...
			_ctrltext = format["Esses sao todo os items que estou vendendo!"];
			ctrlSetText [7412, _ctrltext];
			switch (_lbIndex) do {
			
				case 0: { 
					Z_SellingFrom = 0;
					["Comprando para a mochila."] call Z_filleTradeTitle;
					[0] call Z_calculateFreeSpace;
				};
				case 1: { 
					Z_SellingFrom = 1;
					["Comprando para o veiculo."] call Z_filleTradeTitle;
					_canBuyInVehicle = call Z_CheckCloseVehicle;
					if(_canBuyInVehicle)then{
						[1] call Z_calculateFreeSpace; 
					}else{
						systemChat format["Entre no assento do motorista para vender para seu veiculo."];						
						(_dialog displayCtrl 7404) ctrlSetText format["Espaco Livre: %1 / %2 / %3",0,0,0];
					};
				};
				case 2: {
					Z_SellingFrom = 2;				
					["Comprando para o inventario."] call Z_filleTradeTitle;
	...
	Z_getBackpackItems = {
		...
		if (!isNil "_backpack") then {    				
			...
			_ctrltext = format["Nao sou besta -.-"];
			ctrlSetText [7413, _ctrltext];
			
			_ctrltext = format["Voce nao esta usando uma mochila."];
			ctrlSetText [7412, _ctrltext];
		};
	};
	...
	Z_getVehicleItems = {
		...
		{	
			if(!isNull _x && local _x && !isPlayer _x && alive _x && !(_x isKindOf "zZombie_base"))then{
				systemChat format["Selecionado %1",typeOf _x];
				_vehicle = _x;
			};
		}count _list;
		...
		if(!isNull _vehicle)then{
			...
			_ctrltext = format["Entre no assento do motorista primeiro!"];
			ctrlSetText [7413, _ctrltext];
			
			_ctrltext = format["Nao vejo nenhum veiculo."];
			ctrlSetText [7412, _ctrltext];
		};	
	};
	...
	Z_getGearItems = {
		...
		[_weaps,_mags,"seu inventario"] call	Z_checkArrayInConfig;			
	};
	...
	Z_checkArrayInConfig = {
		...
		if(_total > 0)then{
			...
			_ctrltext = format["Gostaria de oferecer %1 Coins.", _totalPrice,CurrencyName];
			ctrlSetText [7413, _ctrltext];		
			_ctrltext = format["Eu aceito %1 items de %2.", count(Z_SellableArray) , _extraText];
			ctrlSetText [7412, _ctrltext];	
			call Z_fillSellList;
		};		
	};
	...
	Z_calcPrice = {
		...
		_ctrltext = format["%1 Coins", _sellPrice , CurrencyName];
	...
	Z_SellItems = {
		...
		if(_index > -1)then{
			...
			if(typeName _money  == "SCALAR")then{
				...
				systemChat format["Recebido %1 Coins", _money , CurrencyName];			
			}else{
				systemChat format["Dinheiro nao e um numero. Algo esta errado."];			
			};									
		}else{
			systemChat format["Nenhum item para vender"];
		};						
	};
	...
	Z_BuyItems = {	
		...
		if(_myMoney >= _priceToBuy)then{
			...
			if(_canBuy)then{	
			systemChat format["Comecar a comprar para %1 %2",_priceToBuy,CurrencyName];
			
			closeDialog 2;
			
				if(Z_SellingFrom == 0)then{
				systemChat format["Adicionando %1 Items na sua mochila",count (Z_BuyingArray)];
					{
						if( _x select 1 == "trade_weapons")then{
							(unitBackpack player) addWeaponCargoGlobal [_x select 0, _x select 5];
							diag_log format ["%1 x %2 adicionado", _x select 0, _x select 5];							
						};
						if( _x select 1 == "trade_items")then{
				...
				if(Z_SellingFrom == 1)then{
					{
						systemChat format["Adicionando %1 Items em %2",count (Z_BuyingArray), typeOf Z_vehicle];
						if( _x select 1 == "trade_weapons")then{
				...
				if(Z_SellingFrom == 2)then{	
					systemChat format["Adicionado %1 Items no inventario",count (Z_BuyingArray)];
					{
						if( _x select 1 == "trade_weapons")then{
							_count = 0;
							while(_count < _x select 5)do{
								player addWeapon (_x select 0);	
								diag_log format ["%1 adicionado", _x select 0];	
								_count = _count + 1;
							};							 
						};
						if( _x select 1 == "trade_items")then{
				...
				systemChat format["Removendo %1 coins ",_priceToBuy];
				[player,_priceToBuy] call SC_fnc_removeCoins;	
				systemChat format["%1 coins Removidos",_priceToBuy];				
			};				
		}else{
			systemChat format["Voce precisa de %1 coins para comprar esses itens.",_priceToBuy,CurrencyName];
		};
	...
	Z_ChangeBuySell = {		
		...
		Z_Selling = !Z_Selling;	
		if(Z_Selling)then{	
			(_dialog displayCtrl 7416) ctrlSetText "Comprar";
			(_dialog displayCtrl 7409) ctrlSetText "Vendendo";
			...
			(_dialog displayCtrl 7416) ctrlSetText "Vender";
			(_dialog displayCtrl 7409) ctrlSetText "Comprando";
	...
	Z_calculateFreeSpace = {
		...
		if(_selection == 2) then{ //gear
			systemChat format["Nao da para comprar armas ou mochilas para seu inventario ainda, por motivos de seguranca!"];
		...
		_dialog = findDisplay 711197;
		(_dialog displayCtrl 7404) ctrlSetText format["Espaco livre: %1 / %2 / %3",_returnArray select 1,_returnArray select 0,_returnArray select 2];
	};
	...
	Z_CheckCloseVehicle = {	
		...
		if(!isNull _vehicle)then{
			Z_vehicle = _vehicle;
			_result = true;
			[format["Comprando em %1.", typeOf Z_vehicle]] call Z_filleTradeTitle;				
		};						
		_result
	};
	...
	Z_allowBuying = {
		...
		if(_selection == 2) then{ //gear
			systemChat format["No momento, compras de armas ou mochilas para seu inventario esta desabilitado, compre pelo veiculo!"];
			_allowedMags = 20 - count(magazines player);
			_allowedWeapons = 0;
			_allowedBackpacks = 0;	
			...
			if( _allowedWeapons >=_toBuyWeaps)then{
				...
				systemChat format["Voce so pode comprar %1 armas no seu inventario.",_allowedWeapons];
			};
			if( _allowedMags >= _toBuyMags)then{
				_check2 = true;
			}else{
				systemChat format["Voce so pode comprar %1 magazines no seu inventario.",_allowedMags];
			};		
			if( _allowedBackpacks >= _toBuyBags)then{
				_check3 = true;
			}else{
				systemChat format["Voce so pode comprar %1 Mochila  no seu inventario.",_allowedBackpacks];
			};
		...
		if(_selection == 1) then{ //vehicle
			...
			if( _allowedWeapons >= _toBuyWeaps)then{
				...
				systemChat format["Voce so pode comprar %1 armas em seu veiculo.",_allowedWeapons];
			};
			if( _allowedMags >= _toBuyMags)then{
				_check2 = true;
			}else{
				systemChat format["Voce so pode comprar %1 magazines em seu veiculo.",_allowedMags];
			};			
			if( _allowedBackpacks >= _toBuyBags)then{
				_check3 = true;
			}else{
				systemChat format["Voce so pode comprar %1 mochilas em seu veiculo.",_allowedBackpacks];
			};
		...
		if(_selection == 0) then{ //backpack
			...
			systemChat format["Compras para a mochila esta desabilitado por seguranca"];
		};
