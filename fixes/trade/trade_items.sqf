...
if(_buy_o_sell == "buy") then {
	...
	_playerCash = player getVariable ["Chernobyl",0];
	...
	for "_x" from 1 to _total_trades do {
		...
		if (_finished) then {
			if (_playerCash >= _price) then {
				_playerCash = player getVariable ["Chernobyl",0];
				player setVariable["Chernobyl",(_playerCash - _price),true];
				player addMagazine _name;
				_abort = false;
				cutText [format["Pagou %1 %2 por %3",_price,CurrencyName,_textPart], "PLAIN DOWN"];
	...
	_total_trades = _qty;
	if (_total_trades < 1) exitWith {
		cutText [format["Voce nao tem um %1", _textPart], "PLAIN DOWN"];
		call _finish_trade;
	};
	...
	for "_x" from 1 to _total_trades do {
		...
		if (_finished) then {
			...
			if (_removed > 0) then {
				_playerCash = player getVariable ["Chernobyl",0];	
				player setVariable["Chernobyl",(_playerCash + _price),true];
				cutText [format[("Trocou %1 por %2 %3"),_textPart,_price,CurrencyName], "PLAIN DOWN"];
				PVDZE_plr_Save = [player,(magazines player),true,true] ;
				publicVariableServer "PVDZE_plr_Save";
				if (isNil "_name") then { _name = "Unknown Item"; };
				if (isNil "inTraderCity") then {
				...
				cutText [format["Algo deu errado", _name], "PLAIN DOWN"];
				_abort = true;
			};
		};
		sleep 1;
