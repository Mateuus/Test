...
spawnFill = {
	...
	if (count _bodies > 0) then {
		...
		if ((count _block > 0) || {_blockGroup > 0} || {_blockPlot > 0}) then {systemChat format ["Alguns locais estao bloqueados pois um corpo esta perto %1m",_bodyCheck];};
	};
	lbClear _lb;
	{
		_text = _x select 0;
...
spawnPick = {
	private ["_go","_humanity","_spawn","_text","_puid"];
	#include "spawnConfig.sqf"
	_go = 1;
	GET_TEXT
	if (_text == "Near MyGroup") then {_spawn = [0,[],0,0];};
	if (_text == "Near MyPlot") then {_spawn = [0,[1],0,0];};
	if (isNil "_spawn") exitWith {systemChat "Selecione um local!";_go=0;};
	if (count _spawn > 2) then {
		_hlevel = _spawn select 3;
		_humanity = player getVariable ["humanity",0];
		_puid = getPlayerUID player;
		if ((_hlevel < 0) && {_humanity >= _hlevel}) exitWith {systemChat format["Sua humanidade deve ser menor que %1 para esse spawn.",_hlevel];_go=0;};
		if ((_hlevel > 0) && {_humanity <= _hlevel}) exitWith {systemChat format["Sua humanidade deve ser maior que %1 para esse spawn.",_hlevel];_go=0;};
	};
