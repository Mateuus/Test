...
classPick = {
	...
	_go = 1;
	GET_CLASS
	if (isNil "_class") exitWith {systemChat "Selecione uma classe!";_go=0;};
	if (count _class > 9) then {
		...
		_humanity = player getVariable ["humanity",0];
		_puid = getPlayerUID player;
		if ((_hlevel < 0) && {_humanity >= _hlevel}) exitWith {systemChat format["Sua humanidade deve ser menor do que %1 para usar esta classe.",_hlevel];_go=0;};
		if ((_hlevel > 0) && {_humanity <= _hlevel}) exitWith {systemChat format["Sua humanidade deve ser maior do que %1 para usar esta classe.",_hlevel];_go=0;};
		if ((_level == 1) && {!(_puid in _classLevel1)}) exitWith {systemChat "Classe para VIP level 1 somente.";_go=0;};
		if ((_level == 2) && {!(_puid in _classLevel2)}) exitWith {systemChat "Classe para VIP level 2 somente.";_go=0;};
		if ((_level == 3) && {!(_puid in _classLevel3)}) exitWith {systemChat "Classe para VIP level 3 somente.";_go=0;};
	};
