...
if (DZE_ActionInProgress) exitWith { cutText ["Upgrade em progresso." , "PLAIN DOWN"]; };
...
switch (_option) do {
	case "build": {
		...
		if (_id == "") exitWith { cutText ["invalid elevator", "PLAIN"] };
	...
	case "build_stop": {
		_obj = _args select 1;
		if (isNil "ELE_elevator") exitWith { cutText ["Nenhum elevador selecionado", "PLAIN"] };
		_dist = _obj distance ELE_elevator;
		if (_dist > ELE_MaxRange) exitWith { cutText [format["A parada esta muito longe (%1 > %2)", _dist, ELE_MaxRange], "PLAIN"] };
		_id = [ELE_elevator] call ELE_fnc_getNextStopId;
		if (_id == "") exitWith { cutText ["A Parada do elevador ja existe ou tem muitos (max. 9)", "PLAIN"] };
