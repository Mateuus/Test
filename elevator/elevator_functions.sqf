...
ELE_fnc_activateElevator = {
	...
	if (_elevator getVariable ["ElevatorActive", false]) exitWith {
		cutText ["elevador ja ativo", "PLAIN DOWN"];
	};
	_id = _elevator getVariable ["ElevatorID", 0];
	...
	if (isNil "_nextStop") exitWith {
		cutText ["proxima parada nao encontrada", "PLAIN DOWN"];
	};
	_dest = getPosATL _nextStop;
	_dest set [2, (_dest select 2) + 0.05];
	...
	cutText [format["Movendo elevador (%1, %2 m) ...", _nextStopId, _dist], "PLAIN DOWN"];
...
ELE_fnc_callElevator = {
	...
	if (isNil "_elevator") exitWith {
		cutText ["elevador nao encontrado", "PLAIN DOWN"];
	};
	if (_elevator getVariable ["ElevatorActive", false]) exitWith {
		cutText ["elevador ja ativo", "PLAIN DOWN"];
	};
	...
	cutText ["Chamou elevador", "PLAIN DOWN"];
};
